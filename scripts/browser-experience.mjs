...
try {
  ...
  assert.equal((await state()).truthsUnlocked.length, 1); assert.equal((await state()).talks.at(-1).generated, false); assert.ok(await evaluate('document.querySelector(".reply-origin[data-origin=rules]")?.textContent.includes("非模型")')); const before = await state();
