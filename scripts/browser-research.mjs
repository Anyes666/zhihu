...
const output = path.join(ROOT, "artifacts", "zhihu-upgrade"); await mkdir(output, { recursive: true });
...
try {
  server = spawn(process.execPath, ["--import", "./tests/helpers/mock-knowledge-preload.mjs", "server.mjs"], { cwd: ROOT, env: { ...process.env, NODE_ENV: "test", API_LIVE_ENABLED: "true", API_BUDGET_DB: path.join(temp,"budget.sqlite"), API_KNOWLEDGE_DAILY_CALLS: "100", API_KNOWLEDGE_TOTAL_CALLS: "100", PORT: String(port), HOST: "127.0.0.1", LLM_PROVIDER: "none", LLM_API_KEY: "", ZHIHU_ACCESS_SECRET: "", ZHIHU_KNOWLEDGE_ENABLED: "true", ZHIHU_CACHE_DIR: path.join(temp, "cache") }, stdio: "ignore", windowsHide: true });
  ...
  step("桌面首页 / 三封信入口"); await waitFor('document.querySelectorAll(".env").length === 3'); await click("#guide-skip"); await screenshot("01-home");
  await click('.env[data-id="leaving"]'); await click("#go-sort"); await waitFor('document.querySelectorAll(".frag").length === 9');
  ...
  assert.equal((await state()).truthsUnlocked.length, 1); const before = await state();
  step("参考台 / 本地 mock 知识列表（非真实知乎调用） / 明确来源"); await click(".research-desk > summary"); await waitFor('document.querySelectorAll(".research-card[data-capability=knowledge]").length > 0', 20000);
  ...
  report.checks.knowledge = { source: "live-via-local-mock", cards: await evaluate('document.querySelectorAll(".research-card[data-source=live]").length'), channel };
  await screenshot("02-knowledge-desktop", ".research-desk");
  step("相似经历 / 未配置凭证的明确降级 / 自定义检索"); await click('[data-tab="search"]');
  assert.ok((await evaluate('document.querySelector(".research-channel").innerText')).includes("未配置知乎"));
  ...
  report.checks.searchFallback = "NO_SECRET shown; no fabricated results";
  ...
  assert.ok(selected, "本地 mock 应提供明确标注的测试条目");
  await click(`[data-select="${selected}"]`);
  ...
  step("移动端参考台 / 控件边界与保存笔记"); await viewport(390, 844, true); await screenshot("03-knowledge-mobile", ".research-desk");
  ...
  await screenshot("04-write-desktop", ".write");
  ...
  await fill("#reply", reply); await click("#post"); await click("#to-echo");
  ...
  await screenshot("05-ending-desktop", ".echo"); await viewport(390, 844, true); await screenshot("06-receipt-mobile", "#research-receipt");
  const finalOverflow = await evaluate('document.documentElement.scrollWidth <= innerWidth'); assert.equal(finalOverflow, true);
  report.checks.health = await fetch(base + "/api/health").then(r => r.json()); assert.equal(report.checks.health.llm, "none");
