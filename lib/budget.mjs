...
export function createBudget({ env = process.env, dbPath = env.API_BUDGET_DB || path.join(ROOT, "data", "budget", "ledger.sqlite"), now = Date.now } = {}) {
  ...
  const llmUnlimited = env.API_LLM_UNLIMITED === "true";
  ...
  function reserve(kind, { units = 0, actor: suppliedActor } = {}) {
    ...
    const unlimited = kind === "llm" && llmUnlimited;
    if (!policy || !maxConcurrent || !playerPerMinute || !ipPerMinute
      || (!unlimited && (!policy.daily || !policy.total || !policy.player || !policy.ip
    ...
    try {
      ...
      if (!unlimited && (sum.calls >= policy.total || today.calls >= policy.daily)) throw denial("BUDGET_EXHAUSTED");
      if (!unlimited && kind === "llm" && (sum.units + units > totalUnits || today.units + units > dailyUnits)) throw denial("BUDGET_UNITS");
      if (!unlimited && conn.prepare("SELECT count(*) AS n FROM spend WHERE kind=? AND day=? AND player=?").get(kind, day, player).n >= policy.player) throw denial("BUDGET_PLAYER");
      if (!unlimited && conn.prepare("SELECT count(*) AS n FROM spend WHERE kind=? AND day=? AND ip=?").get(kind, day, ip).n >= policy.ip) throw denial("BUDGET_IP");
  ...
  function snapshot() {
    ...
    return { enabled, llmUnlimited, date: day, timezone: "Asia/Shanghai", accounting: "reserved-not-billed", limits: policies,
  ...
  function status() {
    ...
    try {
      ...
      const reason = llmUnlimited ? null : !policy.daily || !policy.total || !dailyUnits || !totalUnits ? "BUDGET_CONFIG"
      ...
      return { mode: reason ? "classic" : "available", reason, message: reason ? budgetMessage(reason) : llmUnlimited ? "模型用量不设应用额度上限；仍保留并发与频率保护，实际输出以本次来源标签为准。" : "实时增强按预算开放；实际输出以本次来源标签为准。" };
