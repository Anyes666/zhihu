...
export function publicState(session) {
  ...
  return {
    ...
    stats: computeStats(session), evidence: evidenceView(session), mailFacts: mailFactsView(session)
  };
}
...
// 封存后的来信事实沿用 applySorting.items 的公开规则，不把错分当成事实。
function mailFactsView(session) {
  if (session.phase === "sort" || !session.sorting) return [];
  return LETTERS[session.letterId].body.flatMap((seg, i) =>
    seg.type === "fact" && (session.sorting[seg.id] === seg.type || session.sortBonus)
...
export function ask(session, charId, q) {
  ...
  const receiptBefore = {
    stamps: session.resources.stamps, trust: before, mood: session.mood[charId],
    truthsUnlocked: [...session.truthsUnlocked], left: session.left.includes(charId)
  };
  ...
  talk.receipt = askReceipt(session, charId, attitude, receiptBefore);
...
// 回执仅比较实际状态；不读取隐藏候选、触发词或模型输出，也不改变行动规则。
function askReceipt(session, charId, attitude, before) {
  const difference = (from, to) => ({ before: from, after: to, delta: to - from });
  const letter = LETTERS[session.letterId];
  const newTruths = session.truthsUnlocked.filter(id => !before.truthsUnlocked.includes(id))
    .map(id => truthView(letter, id, session.truthSources[id]));
  const left = !before.left && session.left.includes(charId);
  const nextActions = [];
  if (session.resources.stamps > 0 && session.summoned.some(id => !session.left.includes(id))) nextActions.push({ id: "ask", label: "继续提问" });
  if (session.resources.summons > 0 && Object.values(CHARACTERS).some(c => c.summonable && !session.summoned.includes(c.id))) nextActions.push({ id: "summon", label: "邀请其他角色" });
  if (session.resources.unlock > 0 && session.truthsUnlocked.length < letter.truths.length) nextActions.push({ id: "unlock", label: "查看档案提示" });
  nextActions.push({ id: "write", label: "开始写回信" });
  return {
    type: "ask", char: charId,
    stamps: difference(before.stamps, session.resources.stamps),
    trust: difference(before.trust, session.trust[charId]),
    attitude: { id: attitude, label: ATTITUDE_LABEL[attitude] },
    mood: { before: before.mood, after: session.mood[charId] }, newTruths, left,
    rules: [
      `本次成功提问消耗 ${before.stamps - session.resources.stamps} 枚邮票。`,
      `规则将问法归为「${ATTITUDE_LABEL[attitude]}」；信任按角色规则调整并限制在 0–100，不是模型心理分析。`,
      newTruths.length ? "本次得到的新证据已列出；只有实际解锁的内容才会公开。" : "本次没有新解锁的证据；这不代表已问清全部情况。",
      ...(left ? ["本次敌意提问后，对方已离开，不能再向该角色提问。"] : [])
    ], nextActions
  };
}
...
export const DEPTH_LABEL = { full: "回信依据覆盖：充分", partial: "回信依据覆盖：部分", blind: "回信依据覆盖：有限" };

// 统一显示口径：确认数量只看实际解锁；覆盖沿用旧评分（猜中也有半分），不能当作确认数量。
export function evidenceView(session) {
  const letter = LETTERS[session.letterId], total = letter.truths.length;
  const count = session.truthsUnlocked.length;
  let coverage = null;
  if (session.reply) {
    const { truthScore, depth } = decideEnding(letter, session);
    coverage = {
      score: truthScore, total, level: depth, label: DEPTH_LABEL[depth],
      confirmedMentioned: letter.truths.filter(t => session.truthsUnlocked.includes(t.id) && session.reply.meta.truthMentions.includes(t.id)).length
    };
  }
  return { confirmed: { count, total, label: `已确认真相 ${count}/${total}` }, coverage };
}
...
export function finalize(session, echoes = []) {
  ...
  const evidence = evidenceView(session);
  ...
  return {
    ...
    missed, persona, scores: s, stats: computeStats(session), timeline: session.timeline, evidence,
    card: { title: letter.title, from: letter.from, persona: persona.title, quote, quoteRisky, quoteLabel: quoteRisky ? "这封回信里最重的那句话" : "我写下的那句话", family: FAMILY_LABEL[family], depth: evidence.coverage.label, evidence, truths: `${session.truthsUnlocked.length}/${letter.truths.length}`, scores: s, date: new Date().toISOString().slice(0, 10), no: String(1000 + (seed % 9000)) }
  };
}
